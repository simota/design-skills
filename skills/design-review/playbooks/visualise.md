...
In this family `location` fires most: a craft finding has a **where**, and
"the card's lower right, under the price" is a sentence the reader has to
translate back into a place on a screen they are looking at.
...
- **A region map** for anything with `location` — the screen divided the way the
  layout divides it, each finding marked where it is, numbered. The default here
