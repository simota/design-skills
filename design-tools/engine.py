...
it: given the engine that is running, it picks one that is not.
    ...
    python3 design-tools/engine.py --running claude --prompt-file p.txt --schema s.json
...
H = yaml.safe_load((ROOT / "design-registry" / "harness.yaml").read_text(endesign="utf-8"))
...
def run(engine: str, prompt: str, schema: dict) -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="design-engine-") as tmp:
        d = pathlib.Path(tmp)
        s = d / "schema.json"
        s.write_text(json.dumps(strict(schema)), endesign="utf-8")
        if engine == "codex":
            out = d / "answer.json"
            argv = ["codex", "exec", "--output-schema", str(s), "-o", str(out),
                    "--sandbox", "read-only", "--skip-git-repo-check", prompt]
            r = _spawn(engine, argv)
            body = out.read_text(endesign="utf-8") if out.exists() else ""
            if not body.strip():
...
def main() -> int:
    ...
    try:
        ...
        got = run(engine,
                  pathlib.Path(a.prompt_file).read_text(endesign="utf-8"),
                  json.loads(pathlib.Path(a.schema).read_text(endesign="utf-8")))
    except EngineError as e:
